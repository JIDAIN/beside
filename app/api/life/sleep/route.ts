...
import { LifeCloudError, deleteSleep, upsertSleep } from "../../../../lib/server/supabase-life";
...
export async function DELETE(request: Request) {
  ...
  if (!validUuid(id)) return lifeJsonError("需要有效的睡眠记录 ID", 400, "INVALID_SLEEP_ID");
  ...
  try {
    const sleep = await deleteSleep(id, partnerKey);
    return NextResponse.json({ ok: true, sleep }, { headers: LIFE_NO_STORE_HEADERS });
  } catch (error) {
    if (error instanceof LifeCloudError) return lifeCloudErrorResponse(error);
    return lifeJsonError("删除睡眠记录失败", 502, "LIFE_WRITE_FAILED");
  }
}
