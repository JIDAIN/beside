    ...
    <AppPageShell title="饮食" subtitle="好好吃饭，也把喜欢的味道留下来。">
      ...
      <div className="grid gap-3">
        {FIXED_MEALS.map(({ type, label, icon }) => {
          ...
          return <section key={type} className="life-surface life-section-card life-meal-card"><div className="mb-3 flex items-center justify-between gap-3"><p className="text-sm font-extrabold text-[var(--life-text)]">{icon} {label}</p>{canEdit ? primary ? <Link href={mealHref(date, partnerKey, type, primary.id)} className="life-meal-edit-icon" aria-label={`编辑${label}`} title={`编辑${label}`}><PencilIcon /></Link> : <Link href={mealHref(date, partnerKey, type)} className="life-meal-add-link">+ 添加{label}</Link> : <span className="rounded-full bg-[var(--life-surface-soft)] px-3 py-1.5 text-[10px] font-bold text-[var(--life-text-muted)]">只读</span>}</div><div className="grid grid-cols-[minmax(0,0.95fr)_minmax(0,1.25fr)] gap-3"><MealPhotoSlot meal={primary} label={label} mealType={type} pending={mealsPending} /><MealNutrition meal={primary} /></div>{extraRecords.length ? <div className="mt-3 grid gap-3 rounded-[var(--life-radius-control)] bg-[var(--life-surface-warm)] p-3"><p className="text-[10px] leading-5 text-[var(--life-text-muted)]">这一天还有 {extraRecords.length} 条同餐次记录，均完整保留并显示。</p>{extraRecords.map((meal, index) => <article key={meal.id} className="rounded-[var(--life-radius-card)] border border-[var(--life-border-soft)] bg-[var(--life-surface)] p-3"><div className="mb-2.5 flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold text-[var(--life-text)]">{label}记录 {index + 2} · {timeLabel(meal)}</p><p className="mt-0.5 line-clamp-1 text-[10px] text-[var(--life-text-muted)]">{mealNames(meal)}</p></div>{canEdit ? <Link href={mealHref(date, partnerKey, type, meal.id)} className="life-meal-edit-icon" aria-label={`编辑${label}记录${index + 2}`} title={`编辑${label}记录${index + 2}`}><PencilIcon /></Link> : null}</div><div className="grid grid-cols-[minmax(0,0.95fr)_minmax(0,1.25fr)] gap-3"><MealPhotoSlot meal={meal} label={`${label}记录${index + 2}`} mealType={type} /><MealNutrition meal={meal} /></div></article>)}</div> : null}</section>;
        })}
