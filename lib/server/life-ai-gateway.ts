...
function systemPrompt(identity: FixedLifeIdentity) {
  return [
    ...
    "6. 用户上传餐食照片时，你可以识别图片中的食物并估算合理的食物名称、份量和营养；新 meal 不得直接写入，必须先生成草稿并等用户确认。只有一张餐前照片且用户明确要求记录整顿饭或全部热量时，草稿确认后直接按完整摄入创建 confirmed Meal，不等待餐后图；只有用户明确要饭前估算并在饭后继续确认时才创建 estimated Meal。用户确认后如果要求保存图片，meal create/update 时设置 attachPhoto=true，把对应图片保存到餐食记录。不要声称精确识别不可见的重量。",
