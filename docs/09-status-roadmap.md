...
当前 Production 存在用户反馈的月历和其他页面读取旧记录、刷新缓慢问题。2026-09-15 已修复共享缓存通知、旧请求竞态和超时恢复等路径，代码待发布；不能把本地测试通过写成线上问题已解决。
...
本轮包含待发布的业务代码：共享查询缓存订阅、GET 超时与重试、月历缓存持久化与隔离、页面前台校验、启动预取削减，以及提醒/AI/恢复后的刷新路径。详细机制见 `04-api-and-sync.md`。

验证：自动测试覆盖共享查询和实际心情/饮食/睡眠月历组件，无需重载即可更新与移除记录；本轮 66 个测试文件、381 项测试通过；Lint 无错误（3 条既有警告），生产构建通过。浏览器访问本地服务被环境拦截（`ERR_BLOCKED_BY_CLIENT`），尚未完成新版本浏览器验收。没有向真实生活记录写入测试数据，也没有执行 migration 或新部署。

下一次获得部署授权后，需在新版本验证记录保存、切月、跨页返回、MCP 写入后前台更新与网络恢复；当前 Production 记录仍为上一轮 9 月 14 日发布。
