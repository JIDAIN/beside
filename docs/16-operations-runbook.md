...
2026-09-15 客户端修复发布后，先核对部署是否包含本次共享查询变更，再按以下路径检查：

1. 确认 API 返回的日期、账号和数据是否正确；接口错误与页面缓存错误分开定位，不先删除生活记录。
2. 浏览器已有缓存时应保留内容并后台校验；GET 超时约 12 秒，共享查询上限 15 秒，失败后自动重试，不能永远复用挂起请求。
3. Web 保存后观察当前页及月历更新；切月和返回日期详情不能被早先请求覆盖。跨标签页同账号应收到变更信号。
4. MCP/其他设备写入后，回到前台触发校验；持续打开的可见页面每 30 秒读取，隐藏页面暂停轮询。网络本身的耗时不包含在 30 秒间隔内。
5. 对心情、睡眠、饮食、体重、药箱、小信箱、提醒、常吃食物及游戏逐个核对其实际读取接口。Legacy Game 有未同步本地修改时应保持保护，不强制远端覆盖。
6. 发布后使用真实浏览器验收；本地 React / CI 测试通过不能替代线上会话、网络和 Service Worker 验收。
