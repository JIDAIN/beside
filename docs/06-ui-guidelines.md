...
- 日期、时间、餐次和加餐时段放在顶部元信息区，允许纠正错误分类；
- 窄屏使用单列，照片与营养合计不挤在同一个双栏卡片；
- 每个食物默认显示名称、实际份量和热量，重量与三大营养素按需展开；
- 所有数字字段保留可见标签和单位，不依赖输入框 placeholder 表达含义；
- 底部保存操作在长表单滚动时保持可达，并避开 safe-area / 主导航；
- 校验错误靠近表单顶部显示，缺食物名称时定位并聚焦对应输入框；
- 有未保存修改时，关闭页面或点击站内导航需要提示；
- 餐食主记录保存成功但照片操作失败时，页面必须转为已有 Meal 并只重试照片，不能重复新增。
...
多项食物只有部分填写某种营养时，该字段的合计显示为“不完整”，不展示部分求和结果冒充整餐总量。
